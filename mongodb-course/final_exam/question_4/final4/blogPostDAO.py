...
class BlogPostDAO:
    ...
    def increment_likes(self, permalink, comment_ordinal):
        try:
            post = self.posts.find_one_and_update({'permalink': permalink}, {'$inc': {'comments.' + str(comment_ordinal) +'.num_likes': 1}}, return_document=ReturnDocument.AFTER)
            print '======'
            print permalink, comment_ordinal, post
            return post.nModified
        except:
            print "Could not like the post, error"
            print "Unexpected error:", sys.exc_info()[0]
            return 0
        return 0
